...
ggplot for all metabolites in the Mineral layer
NOT FINISHED

```{r}
x <- df[which(df$p.category),]
x$sig <- "sig"
df <- merge(df, x, all = TRUE)
# df %>% mutate(group1 = replace(group1, val = "M_C", "Mineral")) %>% mutate(group1 = replace(group1, val = "O_C", "Organic"))
# ggplot(df[which(df$sig == "sig"),], aes(x=Metabolite, y=meancountdifference)) + geom_point() + theme(axis.text.y = element_text(hjust = 0.85)) + ylab("average count difference (H - C)") + xlab("Metabolite") + coord_flip() + ggtitle("Significant Metabolites") + facet_grid(~group1)
```

preliminary figure; not informative - ggplot for all metabolites in the Organic layer

```{r}
ggplot(df[which(df$group1 == "O_C"),], aes(x=Metabolite, y=meancountdifference, color = p)) + geom_point() + coord_flip() + theme(axis.text.y = element_blank())
```
