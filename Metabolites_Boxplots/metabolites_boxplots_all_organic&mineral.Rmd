...
for (i in 1:31) {
 ...
 p = ggplot(new_o_data, aes(x=type, y=frequencies, fill=type, show.legend=FALSE)) +
        ...
        scale_fill_manual(values=c("#f46d43","#d73027")) +
...
p1 <- plot_list[[1]] + theme(plot.title = element_text(size=13)) + theme(plot.background = element_rect(fill = "#74add1"))
p1
p <- plot_grid(p1)
```
```{r}
for (i in c(3,6,8,9,13,23)){
  lplot_list[[i]] <- lplot_list[[i]] + theme(plot.background = element_rect(fill = "#abd9e9"))
}
```
```{r}
...
for (i in c(2,4,7,11,15,20,22,24,26,28,30,31)){
  lplot_list[[i]]<- lplot_list[[i]] + theme(plot.background = element_rect(fill = "#e0f3f8"))
}
...
```
```{r}
m3 <- mp[[3]] + theme(plot.title = element_text(size=13)) + theme(plot.background = element_rect(fill = "#74add1"))
m_3 <- plot_grid(m3)
```
```{r}
for (i in c(7,8,9,11,17,18)){
  lmp[[i]] <- lmp[[i]] + theme(plot.background = element_rect(fill = "#abd9e9"))
}
```
```{r}
bigm <- plot_grid(lmp[[7]], lmp[[8]], lmp[[9]], lmp[[11]], lmp[[17]], lmp[[18]])
mbigm <- plot_grid(m_3, bigm)
mbigm
for (i in c(1,4,5,10,13,19,20,21,22,24,28,29)){
  lmp[[i]]<- lmp[[i]] + theme(plot.background = element_rect(fill = "#e0f3f8"))
}
