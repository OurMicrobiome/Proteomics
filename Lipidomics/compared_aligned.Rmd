...
library(dplyr)
wrc_Wide <- wrc %>%
 distinct(row.identity,keep_all=TRUE) %>%
 pivot_wider(names_from = wrc$type, values_from = frequency, values_fill = list(frequency = 0))
wrc_Wide
wrc_Wides <- spread(wrc,key=type,value=frequency)
```
```{r}
wrc_list <- split(wrc, f=list(wrc$type,wrc$row.identity))
wrc_list[[384]]
```
```{r example t-test p-value}
i <- 1
t.test(wrc_list[[i]][["frequency"]],wrc_list[[3]][["frequency"]], alternative="g")
```
```{r}
foo <- 1:768
...
colnames(dop)
colnames(dop)
Organic <- dop[c(1,6,7,13,14,15,16,18,22,25,29,31,35,36,37,41,44,45,46,47,49),]
