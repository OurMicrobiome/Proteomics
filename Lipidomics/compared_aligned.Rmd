...
In this code, splitn is the list of dataframes for the four groups: Mineral Layer Control, Mineral Layer Heater, Organic Layer Control, and Organic Layer Heated. Each group was normalized and then compared. The p-values were less than 0.05 for 7 of the comparisons and can be found in the dataframe pvalues. 
...
Saved/Created files of all these dataframes and boxplots are:
  ...
  * [Lipidomics/datasets_lipids_in_separate_groups.csv]
...
library(DT)
datatable(splitn[[1]]) # change the 1 to a number 1:52
```

```{r T-test to get p-values where the two samples are H and C for a subset of the array "arrayn"}
